...
```{r}
auditor_merged <- import_ratio_data("./data/CH_auditor_merged.csv")
mainroom_merged <- import_ratio_data("./data/CH_mainroom_merged.csv")
waterdept_merged <- import_ratio_data("./data/CH_waterdept_merged.csv")
engineering_merged <- import_ratio_data("./data/CH_engineering_merged.csv")
veterans_merged <- import_ratio_data("./data/CH_veterans_merged.csv")
clerk_merged <- import_ratio_data("./data/CH_clerk_merged.csv")
```
...
mapply(
  ...
  x=filtered, file=paste("data", paste(names(filtered), "csv", sep="."), sep="/"), #for each dataframe, use its name to make a csv of it
...
mapply(
  write.table, #apply function write table
  x=indoor_src, file=paste("data", paste(names(indoor_src), "csv", sep="."), sep="/"), #for each dataframe, use its name to make a csv of it
  MoreArgs=list(row.names=FALSE, sep=",")
)
```

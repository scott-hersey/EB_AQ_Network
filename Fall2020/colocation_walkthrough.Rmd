...
```{r}
sn45 <- fread("./data/sn45.csv", header = TRUE)
...
```{r}
nm <- list.files(path=cpc_path, all.files = TRUE, full.names = TRUE, pattern="*.TXT") #import and clean CPC 
```
...
```{r}
cpc_upload <- cpc_upload[ , c("X.YY.MM.DD", "HR.MN.SC", "concent", "fillcnt"  )]   ## removing unncessary variables 
```

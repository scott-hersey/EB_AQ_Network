...
```{r}
admin_ratio <- import_ratio_data('../data/filtered/LF_admin_filt.csv')
class1_ratio <- import_ratio_data('../data/filtered/LF_class1_filt.csv')
class2_ratio <- import_ratio_data('../data/filtered/LF_class2_filt.csv', TRUE)
class3_ratio <- import_ratio_data('../data/filtered/LF_class3_filt.csv')
class4_ratio <- import_ratio_data('../data/filtered/LF_class4_filt.csv', TRUE)
```
...
```{r}
write.table(CPC_mean_ratios, "../data/LF_mean_ratios.csv", append = FALSE, sep = ",", dec = ".",
            row.names = FALSE, col.names = TRUE)
write.table(PM_mean_ratios, "../data/LF_pm_mean_ratios.csv", append = FALSE, sep = ",", dec = ".",
            row.names = FALSE, col.names = TRUE)
```
...
# Save image
ggsave("../images/LF_class2_box.png")
```
### Class 4
...
# Save image
ggsave("../images/LF_class4_box.png")
```
...
admin_boxplot
ggsave("../images/LF_admin_box.png")
```
...
class1_boxplot <- ggplot(data=class1_boxplot_ratios, aes(x=source, y=ratio, fill=HEPA_on)) +
  ...
  ylim(0,1)
class1_boxplot
ggsave("../images/LF_class1_box.png")
```
...
class3_boxplot
ggsave("../images/LF_class3_box.png")
```

```{r}
admin_quartile <- admin_boxplot_ratios %>% 
  get_quartile() %>% 
  mutate(room = "admin")
class1_quartile <- class1_boxplot_ratios %>%
  get_quartile() %>%
  mutate(room = "class1")
class2_quartile <- class2_boxplot_ratios %>%
  get_quartile() %>%
  mutate(room = "class2")
class3_quartile <- class3_boxplot_ratios %>%
  get_quartile() %>%
  mutate(room = "class3")
class4_quartile <- class4_boxplot_ratios %>%
  get_quartile() %>%
  mutate(room = "class4")
...
write.table(LF_med, "../data/LF_quartile.csv")

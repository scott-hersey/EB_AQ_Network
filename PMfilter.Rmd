...
We'll create a function that gathers the dates which the wind is coming from the East and exceeds 13 m/s. We also filter those dates for PM values that exceed given values, which are the flag parameters into the function.
...
snfilesTest1 <- snfiles
snfilesTest1$sn45 <- eventFilter(snfiles$sn45, 17, 17, 150)

snfilesTest1$sn49 <- eventFilter(snfiles$sn49, 15, 17, 125)
snfilesTest1$sn62 <- eventFilter(snfiles$sn62, 7, 9, 12)
snfilesTest1$sn67 <- eventFilter(snfiles$sn67, 22, 22, 175)
snfilesTest1$sn72 <- eventFilter(snfiles$sn72, 15, 17, 100)
...
for (i in seq(length(snfiles))){
 generate_PM_polarplots(snfilesTest1[[i]], names(snfiles)[i])
}
# dev.off()
```
...
Now we can save the filtered data that we want to use

```{r}
# mapply(
#   fwrite, #apply function write table
#   x=snfilesTest2, file=paste(names(snfiles), "-final-w-ML-and-PM", ".csv", sep="")
# )
```

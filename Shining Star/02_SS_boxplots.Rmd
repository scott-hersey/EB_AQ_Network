...
```{r}
kitchen_ratio <- import_ratio_data('../data/filtered/SS_kitchen_filt.csv')
front_ratio <- import_ratio_data('../data/filtered/SS_front_filt.csv')
preschool_ratio <- import_ratio_data('../data/filtered/SS_preschool_filt.csv', TRUE)
toddler_ratio <- import_ratio_data('../data/filtered/SS_toddler_filt.csv')
infant_ratio <- import_ratio_data('../data/filtered/SS_infant_filt.csv', TRUE)
```
...
```{r}
write.table(CPC_mean_ratios, "../data/SS_mean_ratios.csv", append = FALSE, sep = ",", dec = ".",
            row.names = FALSE, col.names = TRUE)
write.table(PM_mean_ratios, "../data/SS_pm_mean_ratios.csv", append = FALSE, sep = ",", dec = ".",
            row.names = FALSE, col.names = TRUE)
```
...
# Save image
ggsave("../images/SS_preschool_box.png")
```
### Infant
...
# Save image
ggsave("../images/SS_infant_box.png")
```
...
kitchen_boxplot
ggsave("../images/SS_kitchen_box.png")
```
...
front_boxplot <- ggplot(data=front_boxplot_ratios, aes(x=source, y=ratio, fill=HEPA_on)) +
  ...
  ylim(0,1)
front_boxplot
ggsave("../images/SS_front_box.png")
```
...
toddler_boxplot
ggsave("../images/SS_toddler_box.png")
```

```{r}
kitchen_quartile <- kitchen_boxplot_ratios %>%
  get_quartile() %>%
  mutate(room = "kitchen")
front_quartile <- front_boxplot_ratios %>%
  get_quartile() %>% 
  mutate(room = "front")
preschool_quartile <- preschool_boxplot_ratios %>%
  get_quartile() %>% 
  mutate(room = "preschool")
toddler_quartile <- toddler_boxplot_ratios %>%
  get_quartile() %>%
  mutate(room = "toddler")
infant_quartile <- infant_boxplot_ratios %>%
  get_quartile() %>%
  mutate(room = "infant")
...
write.table(SS_med, "../data/SS_quartile.csv")

...
This is work I did to try to stop NO sensor oscillations that were products of RH changing (ie unrepresentative data). We didn't end up using any of this work because Eben came up with an ML solution that fixed it. I am including this file in the repo as reference. I also updated the file paths used in this file.
...
```{r}
sn45 <- read.csv("data/sn45.csv") # import modified sensor data, taken from Sensor Data dropbox, and downloaded locally to data folder in this repo
...
```{r}
source("functions/creategraph.R")
...
creategraph(sn45, sn45$date, sn45$o3, sn45$no2, sn45$no, sn45$rh_manifold)
creategraph(septoct_period3, septoct_period3$date, septoct_period3$o3, septoct_period3$no2, septoct_period3$no, septoct_period3$rh_manifold)
creategraph(oct_period1, oct_period1$date, oct_period1$o3, oct_period1$no2, oct_period1$no, oct_period1$rh_manifold)
```

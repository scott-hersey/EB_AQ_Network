...
sn_arrivals <- sn45[sn45$date %within% intervals_arrival$intervals_list[1] , ] #extracting all the sensor data in the first interval
for (indx in 2:length(intervals_arrival$intervals_list)){
  sn_arrivals <- rbind(sn_arrivals, sn45[sn45$date %within% intervals_arrival$intervals_list[indx] , ]) 
}

sn_departures <- sn45[sn45$date %within% intervals_departures$intervals_list[1] , ]
for (indx in 2:length(intervals_departures$intervals_list)){
  sn_departures <- rbind(sn_departures, sn45[sn45$date %within% intervals_departures$intervals_list[indx] , ]) 
}

sn_other <- sn45[sn45$date %within% intervals_other$intervals_list[1] , ]
for (indx in 2:length(intervals_other$intervals_list)){
  sn_other <- rbind(sn_other, sn45[sn45$date %within% intervals_other$intervals_list[indx] , ]) 
}
```
...
```{r}
write.csv(sn_reg1, "sn45_reg1.csv")
write.csv(sn_departures, "sn45_reg2_D.csv")
write.csv(sn_arrivals, "sn45_reg3_A.csv")
write.csv(sn_reg4, "sn45_reg4.csv")
```
...
length(sn45$date) == sum(length(sn_departures$date) + length(sn_arrivals$date) + length(sn_reg1$date) + length(sn_reg4$date))
length(sn45$date) == sum(length(sn_departures$date) + length(sn_arrivals$date) + length(sn_other$date) )

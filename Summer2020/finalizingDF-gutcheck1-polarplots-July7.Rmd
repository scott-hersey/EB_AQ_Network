...
createsensordf <- function(sensorpath, no2modelpath, metdf){
  ...
  sensor[, c("lat", "lon", "X", "X.1", "sn")] <- list(NULL) #removing columns for speed
...
```{r}
source("functions/createsensordf.R")
metdata <- createmetdf("data/metdata.csv") #this is the meteorology data from ASOS website. This file can be found on the Sensor Data DropBox folder

#the following files come from Eben's dropbox that he emailed us
...
```{r}
sn67 <- read.csv("data/sn67.csv") #since we're storing all data in the data folder, we're going to call these modified sensor dataframes from that folder
sn67$X <- NULL
sn67$date <- ymd_hms(sn67$date)

sn62 <- read.csv("data/sn62.csv")
sn62$X <- NULL
sn62$date <- ymd_hms(sn62$date)

sn49 <- read.csv("data/sn49.csv")
sn49$X <- NULL

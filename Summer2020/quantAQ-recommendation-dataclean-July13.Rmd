...
Update: In this notebook, I kept some of the preliminary code that implements baseline correction on NO. However, it is not yet finished and I only include it for reference. I polished some of the baseline code from this notebook, put it in the baseline-reportout Rmd, and deleted it from this notebook.
...
In order to determine the magnitude of the no_ae derivative to filter by, let's look at the average derivative and std . Below, I include a function that filters NO based on no_ae derivatives that are some multiple of the standard deviation.


```{r}
no_ae_filter <- function(sn, mag){
  #sn -> sensor dataframe 
  #mag -> magnitude, an integer to multiply the standard deviation by
...
Let's try it for no_ae derivatives that are three times the standard deviation: 
...
```
So, originally, there was 132450 negative NO points, but now there's 457 points less. 
...
For now, I'll just run the function I created in June, that takes the original datafiles in and downsamples them. 
...
So, almost half of the datapoints are negative. However, are the all within the standard deviation of NO? within the standard error? Note that the RMSE given by the QuantAQ website is about 30 ppb for sn45's NO sensor. 

```{r}
min(negative_NO, na.rm= TRUE)
```
It's about 10 ppb over the RMSE and three times the standard deviation of NO.

The average negative NO value is:
...
```
Since the negative data is not large, compared to the plume spikes, it's not really affecting the story about plumes. However, it would affect the long term story. 
...
timePlot(selectByDate(sn45_NOfilter_3SD, start = "6/9/2019", end="30/9/2019"), pollutant=c("filteredNOPOS", "no"), y.relation = "free")
timePlot(selectByDate(sn45_NOfilter_3SD, start = "6/9/2019", end="18/9/2019"), pollutant=c("filteredNOPOS", "no"), y.relation = "free")
timePlot(selectByDate(sn45_NOfilter_3SD, start = "12/9/2019", end="18/9/2019"), pollutant=c("filteredNOPOS", "no"), y.relation = "free")
...
timePlot(selectByDate(sn45_NOfilter_3SD, start = "4/12/2019", end="18/12/2019"), pollutant=c("filteredNOPOS", "no"), y.relation = "free")

#to compare to before plots
timePlot(selectByDate(sn45_NOfilter_3SD, start = "14/2/2020", end = "15/2/2020"), pollutant = c("filteredNOPOS", "no"), main = "NO vs Time for SN45", y.relation = "free")
timePlot(selectByDate(sn45_NOfilter_3SD, start = "24/2/2020", end = "29/2/2020"), pollutant = c("filteredNOPOS", "no"), main = "NO vs Time for SN45", y.relation = "free")
timePlot(selectByDate(sn45_NOfilter_3SD, start = "6/4/2020", end = "4/5/2020"), pollutant = c("filteredNOPOS", "no"), main = "NO vs Time for SN45", y.relation = "free")
timePlot(selectByDate(sn45_NOfilter_3SD, start = "8/5/2020", end = "18/5/2020"), pollutant = c("filteredNOPOS", "no"), main = "NO vs Time for SN45", y.relation = "free")
```
...
One strategy that we could use, based on this information, is to take everything -2<x<0 ppb, and set it to zero. That way, we're approximating what the value actually is without losing as much resolution. 
...
```
```{r}
(length(negative_NO) - sum(abs(negative_NO) < 2 , na.rm=TRUE )  ) / length(sn45_NOfilter_3SD$no)
```

~34% of all the negative vales are less than -2 ppb. 
If we'd set those to zero instead of NA, we'd get back about a third of the points lost and overall, we'd lose 29% of datapoints.
...
Let's see if we can subtract the polynomial 


Below, I'll try to extract the NO data and date as well, then calculate the polynomial.  
...
First, I try the examples from the baseline package information page and [stackoverflow](https://stackoverflow.com/questions/37346967/r-packagebaseline-application-to-sample-dataset).
...
Now, let's take a day's worth of data from our sensor to try this method on : 
```{r}
baselinetest1 <- selectByDate(sn45_NOfilter_3SD, start = "8/5/2020", end = "8/5/2020")

```
Now let's try using the baseline function: 

```{r}
# this doesn't work bc it has to be matrix form -> bc.irls <- baseline(baselinetest1$no)
baselinetest1.t <- t(baselinetest1$no)
BT.fillPeaks <- baseline(baselinetest1.t, lambda=6,
                     hwi=50, it=10, int=2000, method='fillPeaks')
plot(BT.fillPeaks)
```

```{r}
baselineBT <- getBaseline(BT.fillPeaks)
correctedBT <- getCorrected(BT.fillPeaks)
```

```{r}
plot(baselinetest1$date, baselinetest1$no, type="l")
plot(baselinetest1$date, correctedBT, type = "l")
```

It looks like it did return it with the same indices! However, it made the highest peak larger.




Code for a moving filter that corrects day-by-day:


```{r}
for(day in 8:18){
  nodate2 <- selectByDate(NO_Date, start = paste0(day,"/9/2019"), end=paste0(day,"/9/2019"))
  nodate2.t <- t(nodate2$no)
  bc.peakDetection <- baseline(nodate2.t, method='peakDetection',left=300, right=300, lwin=50, rwin=50)
  
  corrected <- getCorrected(bc.peakDetection)
  nodate2$corrected <- t(corrected)

  sn45$corrected <- ifelse(sn45$date %in% nodate2$date, nodate2$corrected, sn45$no)
}
```

...
In this notebook, I created a dataframe with upsampled meteorological data and tested the NO2 model. This was initial work I did in order to figure out how to implement these things, but the finalized versions of these are in finalizingDF-gutcheck1-polarplots-July7.Rmd. Therefore, the purpose of this notebook is just as a record. 
...
```{r}
sensorpath <- "data/sn45.csv" #modified sensor data, taken from Sensor Data dropbox, saved locally in data folder of this repo
asospath <- "data/metdata.csv" #original meteorology data, taken from sensor data dropbox
...
```{r}
no2model <- read.csv("no2_HYBRID_045.txt") #taken from Eben's dropbox, which he sent via email 

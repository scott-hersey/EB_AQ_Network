...
boxfiles = sapply(temp,  FUN = function(x) fread(file=x,  data.table = TRUE, select = final_vars), simplify = FALSE,USE.NAMES = TRUE) #importing them
```
...
temp2 = list.files(path = "./data/quant_aq_March2021", pattern = "^.*_final.*\\.csv", full.names = TRUE)
...
temp3 = list.files(path = "./data/quant_aq_March2021", pattern = "^.*_raw.*\\.csv", full.names = TRUE)
...
```{r}
# joined_files <- sapply(names(cut_quantAQfiles), function(x) rbind(boxfiles$x, cut_quantAQfiles$x, fill = TRUE), simplify = FALSE,USE.NAMES = TRUE)
# 
#   joined_files <- do.call("rbind", apply(boxfiles, 1, gen.data))
```
...
sn49joined <- rbind(boxfiles$sn46, cut_quantAQfiles$sn49, fill = TRUE)
sn62joined <- rbind(boxfiles$sn46, cut_quantAQfiles$sn62, fill = TRUE)
sn67joined <- rbind(boxfiles$sn46, cut_quantAQfiles$sn67, fill = TRUE)
sn72joined <- rbind(boxfiles$sn46, cut_quantAQfiles$sn72, fill = TRUE)
...
```{r}
lapply(joined_files, function(x) plot(x$date, x$o3))
```
...
mapply(
  ...
  x=joined_files, file=paste(names(joined_files), "csv", sep="."), #for each dataframe, use its name to make a csv of it

...
boxfiles = sapply(temp,  FUN = function(x) fread(file=x,  data.table = TRUE, select = final_vars), simplify = FALSE,USE.NAMES = TRUE) 
```

```{r}
new_names <- lapply(names(boxfiles), function(x) substr(x, 16, 25)) #extract the real sensor names from the current names used in the list

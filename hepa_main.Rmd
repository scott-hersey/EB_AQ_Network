...
engineering_PM <- do.call(cbind, lapply(list.files(path="./City Hall Data - Jan 2021/Veterans AL MOD030", pattern="*.csv", full.names = TRUE), function(x) fread(file=x, header = TRUE))) 

clerk_PM <- do.call(cbind, lapply(list.files(path="./City Hall Data - Jan 2021/Veterans AL MOD030", pattern="*.csv", full.names = TRUE), function(x) fread(file=x, header = TRUE))) 
...
engineering_ratio <- ratio_merge(pm_list$engineering, rooms$outdoor_ch, cpc = FALSE)
...
clerk_ratio <- ratio_merge(pm_list$clerk, rooms$outdoor_ch, cpc = FALSE)
```
...
```{r}
ratios <- list("engineering_ratio" = engineering_ratio, "waterdept_ratio" = waterdept_ratio, "mainroom_ratio" = mainroom_ratio, "auditor_ratio" = auditor_ratio, "veterans_ratio" = veterans_ratio, "clerk_ratio" = clerk_ratio)
```
...
# Making sure the files read in correctly 

```{r}
read_in_files <- list.files(path = ".", pattern = "*.csv", full.names = TRUE)

ratios_read_in <- sapply(read_in_files, function(x) fread(file= x, header = TRUE, data.table = TRUE), USE.NAMES = TRUE)

names(ratios_read_in)<- lapply(names(ratios_read_in), function(x) substr(x, 3, 11))
```

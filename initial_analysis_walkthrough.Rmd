...
temp2 = list.files(path = "./", pattern = "*.csv", full.names = TRUE) #specify files we're looking for
...
```{r}
#sn45cleaning <- fread("./data/quant_aq/SN45_NaN.*\\.csv", header = TRUE) #get the igor file for sn45
...
It looks like sn67 has one negative NO value, so we will inspect it it: 

```{r}
min_indx <- which.min(snfiles$sn67$correctedNO)

```

```{r}
timePlot(snfiles$sn67[(min_indx - 1): (min_indx + 3) ,], pollutant = c("correctedNO", "no2"), y.relation = "free")
```

Upon closer inspection, it looks like it is a period of time when the data is negative, so we'll remove the chunk: 

```{r}
snfiles$sn67[(min_indx-1):(min_indx+3), "correctedNO"] <- NA
```

It also looks like sn72 has some high values: 

```{r}
library(ggplot2)
max_indx <- which.max(snfiles$sn72$no2)

ggplot(data=snfiles$sn72[(max_indx - 7): (max_indx + 5),], aes(x=date_local, y=no2)) +
  geom_line()+
  geom_point()

ggplot(data=snfiles$sn72[(max_indx - 7): (max_indx + 5),], aes(x=date_local, y=correctedNO)) +
  geom_line()+
  geom_point()
#timePlot(snfiles$sn72[(max_indx - 7): (max_indx + 10),], ("correctedNO", "no2"))
View(snfiles$sn72[(max_indx - 10): (max_indx + 10), c("date_local", "correctedNO", "no2")])
```
...
for (i in seq(length(names(snfiles)))){
  ...
  timeVariation(snfiles[[i]], pollutant = c("pm1", "co", "no", "bin0", "bin1"), normalise = TRUE, main = paste0(names(snfiles)[i], " Normalized Group of Pollutants Diurnal Profile"))
}
```
...
```{r}
snfiles$sn45_before <- snfiles$sn45[date < "2020-03-01 00:00:00 EST"]
snfiles$sn45_after <- snfiles$sn45[date > "2020-04-01 00:00:00 EST"]

snfiles$sn46_before <- snfiles$sn46[date < "2020-03-01 00:00:00 EST"]
snfiles$sn46_after <- snfiles$sn46[date > "2020-04-01 00:00:00 EST"]

snfiles$sn49_before <- snfiles$sn49[date < "2020-03-01 00:00:00 EST"]
snfiles$sn49_after <- snfiles$sn49[date > "2020-04-01 00:00:00 EST"]

snfiles$sn62_before <- snfiles$sn62[date < "2020-03-01 00:00:00 EST"]
snfiles$sn62_after <- snfiles$sn62[date > "2020-04-01 00:00:00 EST"]

snfiles$sn67_before <- snfiles$sn67[date < "2020-03-01 00:00:00 EST"]
snfiles$sn67_after <- snfiles$sn67[date > "2020-04-01 00:00:00 EST"]

snfiles$sn72_before <- snfiles$sn72[date < "2020-03-01 00:00:00 EST"]
snfiles$sn72_after <- snfiles$sn72[date > "2020-04-01 00:00:00 EST"]
```

...
```{r}
names(finalfiles)<- lapply(names(finalfiles), function(x) substr(x, 18, 21)) #extract the real sensor names from the current names used in the list
...
```{r}
metdata <- fread("data/Oct_2020_Met_EST.csv", header=TRUE, data.table = TRUE) #import meteorology file
```
...
```{r}
snfiles <- lapply(snfiles, function(x) x[date > "2020-09-07 00:00:00 EST"])
```
...
```{r}
flags_list = list(c(2,"co2"), c(3,"no2"), c(6, "no"))
for(i in flags_list){
...
names(snfiles_correction) <- lapply(names(snfiles), function(x) substr(x, 17, 26)) #change the names
names(snfiles) <- lapply(names(snfiles), function(x) str_replace(x, "SN", "sn"))
```
...
snfiles <-  sapply(names(snfiles), function(k) apply_igor(snfiles_correction[[k]], snfiles[[k]])) #apply the igor corrections, by matching corrections and snfiles by name

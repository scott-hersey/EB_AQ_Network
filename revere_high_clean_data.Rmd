...
outdoor_class_files = list.files(path="./Revere High School - Mar 2021/Class120_outdoor/MCPC 016 - outdoor 22mar-24mar", pattern="*.TXT", full.names = TRUE)
outdoor_class_cpc <- do.call(rbind,
                        lapply(outdoor_class_files, function(x) fread(file=x, header = TRUE, data.table = TRUE, skip="#YY/MM/DD", fill=TRUE)))
...
business_PM <- do.call(cbind, lapply(list.files(path="./Revere High School - Mar 2021/Business - CPC077 MOD099", pattern="*.csv", full.names = TRUE), function(x) fread(file=x, header = TRUE)))
...
# principal_PM <- do.call(cbind, lapply(list.files(path="./Revere High School - Mar 2021/Principal - In_Out CPC016 MOD114 MOD126", pattern="MOD126.*.csv", full.names = TRUE), function(x) fread(file=x, header = TRUE)))

principal_PM <- do.call(cbind, lapply(list.files(path="./Modulair-PM Data", pattern="104.*.csv", full.names = TRUE), function(x) fread(file=x, header = TRUE)))
...
class210_PM <- do.call(cbind, lapply(list.files(path="./Modulair-PM Data", pattern="111.*.csv", full.names = TRUE), function(x) fread(file=x, header = TRUE)))

classc11_PM <- do.call(cbind, lapply(list.files(path="./Modulair-PM Data", pattern="099.*.csv", full.names = TRUE), function(x) fread(file=x, header = TRUE)))

class120_PM <- do.call(cbind, lapply(list.files(path="./Modulair-PM Data", pattern="126.*.csv", full.names = TRUE), function(x) fread(file=x, header = TRUE)))

outdoor_class_PM <- do.call(cbind, lapply(list.files(path="./Modulair-PM Data", pattern="114.*.csv", full.names = TRUE), function(x) fread(file=x, header = TRUE)))
...
```{r}
pm_admin_list$outdoor <- pm_admin_list$outdoor %>%
  filter(date < as.POSIXct("2021-03-11 06:36:00", tz='America/New_York') | 
         date > as.POSIXct("2021-03-11 14:08:00", tz='America/New_York'))

pm_classroom_list$outdoor <- pm_admin_list$outdoor %>%
  filter(date < as.POSIXct("2021-03-11 06:36:00", tz='America/New_York') | 
         date > as.POSIXct("2021-03-11 14:08:00", tz='America/New_York'))
```
